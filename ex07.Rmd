...
Here's a link to a .csv file containing the data:  [[ortann_csv]](https://pjbartlein.github.io/GeogDataAnalysis/data/csv/ortann_csv).
...
There are three arguments that must be passed to the function:  1) the map title (a string), 2) a lable for the variable (a string), 3) the name of the variable (a string), and 4) the variable-value limits (a pair of values, concatenated with `c()`)

Plot the values of TJanDiff by setting the values of the arguements, and then calling the `map_tmp()` function:
