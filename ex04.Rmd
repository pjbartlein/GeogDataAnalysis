...
```{r echo=TRUE, eval=FALSE}
coplot(orstationc$tann ~ eorstationc$lev | orstationc$lon * orstationc$lat, number=5, overlap=.5,
    panel=function(x,y,...) {
...
```{r echo=TRUE, eval=FALSE}
cloud(orstationc$Elev ~ orstationc$Lon*orstationc$Lat, pch=16, cex=1.25, col=unclass(as.factor(orstationc$Glacier)))
```
...
```{r echo=TRUE, eval=FALSE}
Lat2 <- equal.count(orstationc$Lat,4,.5)
Lon2 <- equal.count(orstationc$Lon,4,.5)
plot(Lat2)
plot(Lon2)
```
...
plot2 <- xyplot(orstationc$Elev ~ Lon | Lat2,
    layout = c(4, 2),
    panel = function(x, y) {
        panel.grid(v=2)
        panel.xyplot(x, y)
...
```{r echo=TRUE, eval=FALSE}
# Lattice stripplot
plot4 <- stripplot(Gorstationc$lacier ~ orstationc$Elev | Lat2*Lon2)
print(plot4)

# Lattice boxplot
plot5 <- bwplot(orstationc$Glacier ~ orstationc$Elev| Lat2*Lon2)
print(plot5)
```
